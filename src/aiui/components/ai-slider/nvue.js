...
export default {
    ...
    methods: {
        ...
        // 移动点占总长度的百分比，此处需要先除以step，是为了保证step大于1时，比如10，那么在滑动11,24rpx这样的
        // 距离时，实际上滑块是不会滑动的，到了16,34rpx，经过四舍五入后，就变成了40rpx，进行了下一个跳变
        format(value) {
            return Math.round(uni.$util.range(this.min, this.max, value) / this.step) * this.step
        },
        getRange() {
            const {
                max,
