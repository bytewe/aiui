  ...
  methods: {
    ...
    onTouchstart(e) {
      ...
      this.panEvent = bindingX.bind({
        ...
        if (res.state === 'end' || res.state === 'exit') {
          ...
          } else if(Math.abs(deltaX) > uni.$util.getPx(this.threshold)) {
    ...
    queryRect() {
      ...
      const promiseAll = this.options.map((item, index) => {
        return this.getRectByDom(this.$refs[`ai-swipe-action-item__right__button-${index}`][0])
      })
    ...
    moveCellByAnimation(status = 'open') {
      ...
      animation.transition(content, {
        ...
        duration: uni.$util.getDuration(this.duration, false),
    ...
    // 获取元素ref
    getContentRef() {
      return this.$refs['ai-swipe-action-item__content'].ref
    },
    beforeDestroy() {
      this.unbindBindingX()
    }
